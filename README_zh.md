...
- **系统健壮性**: AI 熔断器、结构化失败指标、点击失败冷却机制、进程自动重启（指数退避）([详情](docs/implementation/2026-04-09-system-robustness-fixes.md))
- **监控基础设施**: SQLite 心跳服务、三层 AI 健康检查、`/api/monitoring/*` REST API
- **Sidecar 夜间模式超时**: 22:00–08:00 可配置 30 秒超时（白天 300 秒）
