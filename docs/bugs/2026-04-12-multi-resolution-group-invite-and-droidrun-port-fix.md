...
Added `_screen_width` / `_screen_height` instance variables with `_update_screen_dimensions()` that detects screen size from the UI tree root element's bounds. Called inside each retry attempt of `open_chat_info`, `tap_add_member_button`, `search_and_select_member`, and `confirm_group_creation` (these methods also **retry up to 3 times** on tap / element miss, merged with the multi-device reliability branch).

`navigate_to_chat` still calls `_ensure_screen_resolution()` so **scroll / swipe** coordinates stay scaled to the physical device relative to the 1080×2340 reference; group-invite **pixel heuristics** use the tree-updated width/height with **ratio-based** thresholds (not fixed 720p numbers).
