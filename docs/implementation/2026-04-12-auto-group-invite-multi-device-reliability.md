...
4. **Resolution-aware automation**: `services/wecom_service.py` — read device resolution, derive scale factors from a **1080×2340 reference**, scale swipe/tap/bounds thresholds used by group-invite and related flows; refresh scroll coordinates from config; add **retries + structured logging** on critical steps (`open_chat_info`, `tap_add_member_button`, `search_and_select_member`, `confirm_group_creation`).
