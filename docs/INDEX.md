...
> Last Updated: 2026-04-12 (Auto group invite: multi-resolution UI bounds + UI-tree sizing, scroll scaling via `_ensure_screen_resolution`, retries on chat-info / add-member / search / confirm; DroidRun per-device ports; **Private Chats** restore after realtime auto-invite; diagnostics `diagnose_group_invite` + `run_group_invite_*`; see `bugs/2026-04-12-*.md` and `implementation/2026-04-12-auto-group-invite-multi-device-reliability.md`)
