...
| [Media Auto-Actions](features/media-auto-actions.md)                                                               | ✅ Complete | 2026-04-30 | Auto-blacklist + Android group invite + auto contact share (主管名片); `ui_parser` structural media detection; `WeComService` scroll scaling + UI-tree screen sizing + ratio bounds + step retries; `diagnose_group_invite` / `run_group_invite_*`; `test-trigger` still no real UI without `WeComService` |
