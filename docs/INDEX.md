...
> Last Updated: 2026-04-28 (AI 熔断器健康检查锁死修复；见 `bugs/2026-04-27-circuit-breaker-skip-reply.md`；前一更新 2026-04-21 Sidecar / Logs WebSocket 自动重连与分层保活)
...
| [Media Auto-Actions](features/media-auto-actions.md)                                                               | ✅ Complete | 2026-04-12 | Auto-blacklist + Android group invite; `ui_parser` structural media detection; `WeComService` scroll scaling + UI-tree screen sizing + ratio bounds + step retries; `diagnose_group_invite` / `run_group_invite_*`; `test-trigger` still no real UI without `WeComService` |
