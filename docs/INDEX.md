...
> Last Updated: 2026-05-17 (**Realtime reply hot-chat priority** — two-tier `hot_queue` / `cold_queue` in `response_detector` so users who already chatted this scan cycle are clicked before new strangers; see [implementation/2026-05-17-realtime-reply-hot-chat-priority](implementation/2026-05-17-realtime-reply-hot-chat-priority.md). Earlier: 2026-05-16 **Device-dashboard remote commands** — `HeartbeatClient` reader/writer split removes ~10s command latency; see [resolved/2026-05-16-dashboard-remote-command-latency](04-bugs-and-fixes/resolved/2026-05-16-dashboard-remote-command-latency.md). Earlier: 2026-05-12 **Media auto-actions / image review SSOT** — `review_gate` no longer stores server URL; single default schema in `settings_loader`; migration + tests in [implementation/2026-05-12-media-actions-settings-dedup-ssot](implementation/2026-05-12-media-actions-settings-dedup-ssot.md). Earlier same day: **Photo-rate / click funnel** — new-friend keyword parity + day-level click blocklist + relaxed list matching + `click_health` monitoring API + `monitoring.db` table; **plus** unread row parsing confidence so message snippets like `你好` are not emitted as customer names, and a low-confidence priority filter in `response_detector`. See `implementation/2026-05-12-new-friend-click-loop-photo-rate-remediation.md` and `04-bugs-and-fixes/resolved/2026-05-12-new-friend-false-positive-click-loop.md`. Earlier: 2026-05-11 pre-group message + media action reorder + log-level fix.)
...
| ------------------------------------------------------------------------------------------------------------------ | ----------- | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Dashboard remote-command latency fix](04-bugs-and-fixes/resolved/2026-05-16-dashboard-remote-command-latency.md) | ✅ Complete | 2026-05-16 | `HeartbeatClient` dedicated reader loop; dashboard start/stop/pause no longer waits for 10s heartbeat interval                                                                                                                                                             |
| [Auto Contact Share (自动推送主管名片)](features/auto-contact-share.md)                                            | ✅ Complete | 2026-05-09 | 2026-05-07 UI 可靠性 + **2026-05-09** `review_gate`：`evaluate_gate_pass`、双库（会话库 vs 控制库）、`image`/`video` 处理器 `db_path`、可观测日志；实现 [2026-05-09-contact-share-review-gate](implementation/2026-05-09-contact-share-review-gate.md)；E2E `scripts/e2e_verify_contact_picker_state.py` + dry-run `tests/integration/test_full_image_to_card_dry_run_e2e.py` |
...
| [Media Auto-Actions](features/media-auto-actions.md)                                                               | ✅ Complete | 2026-05-12 | Auto-blacklist + Android group invite + auto contact share; **2026-05-12** 审核服务器地址/超时与系统设置统一（SSOT），`review_gate` 仅门控与视频策略；实现 [2026-05-12-media-actions-settings-dedup-ssot](implementation/2026-05-12-media-actions-settings-dedup-ssot.md)。此前：2026-04-30 起 UI 解析、拉群、名片等 |
...
- [Auto contact share + review gate (E2E, dual DB)](implementation/2026-05-09-contact-share-review-gate.md) - `evaluate_gate_pass`, `build_media_event_bus` session vs control DB, image/video review persist path, log keywords, dedupe limitation; **2026-05-12** log line wording updated for `general.image_server_ip` SSOT
- [Media auto-actions: image review settings SSOT (dedup + migration)](implementation/2026-05-12-media-actions-settings-dedup-ssot.md) - Remove duplicate `review_gate` URL/timeout; `settings_loader` single default schema; `SettingsService` migration; `build_review_components` reads `general.*`
...
- [New-friend click loop + photo-rate remediation](implementation/2026-05-12-new-friend-click-loop-photo-rate-remediation.md) - Keyword parity, `_click_dayblock`, list matching + scroll floor, `click_health` API + DB, unread list **row parsing confidence** (no fake `你好` targets), priority low-confidence filter, regression tests (`test_unread_user_extractor_row_parsing`, dayblock tests)
...
- [Sidecar review timeout defaults](sidecar/sidecar-review-timeout-defaults.md) - Day vs night wait timeouts and code locations
...
- [New-friend false positive + click loop starving photo-rate funnel](04-bugs-and-fixes/resolved/2026-05-12-new-friend-false-positive-click-loop.md) — removed broad `感谢您` match; `sync_service` / `unread_detector` keyword parity tests; `_click_dayblock` at priority-detect time; `WeComService` list matching + min scroll floor; `GET /api/monitoring/click-health` + `click_health` table; **05-10** message-as-name root fix (two `UnreadUserExtractor`s + priority sanity filter); implementation [2026-05-12-new-friend-click-loop-photo-rate-remediation](implementation/2026-05-12-new-friend-click-loop-photo-rate-remediation.md)

#### 2026-04

- [AI Circuit Breaker locked by health checker](bugs/2026-04-27-circuit-breaker-skip-reply.md) - Severity-aware `force_open` policy; `inference_timeout` no longer trips the breaker; N=2 consecutive gate for `service_down`/`inference_error`
